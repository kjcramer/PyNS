# -*- coding: utf-8 -*-
"""
Postprocessing script for simulations without coolant:
Loads the .npz files
...
yc = np.append(avg(yn_fil),-0.0005) # location of condensation interface
...
t_plot_m=np.append(t_fil[x_mid,:,z_pos],t_int_film[x_mid,:,z_pos])
...
t_plot_e=np.append(t_fil[x_end,:,z_pos],t_int_film[x_end,:,z_pos])
...
plt.plot([18, 80],[-0.0005, -0.0005], linestyle='--', color='black', linewidth=1)
#plt.xlim([18, 80])
...
y,x = np.mgrid[0:5:1,0:55:1]
#y,x = np.mgrid[0:0.125:0.00390625,0:1.25:0.0048828125]
uu = np.transpose(u_air[:,0:5,z_pos])
vv = np.transpose(v_air[0:55,:,z_pos])
ww = np.transpose(w_air[0:55,0:5,z_pos])
...
#%% contour plots for normal configuration, reordering necessary for reversed
