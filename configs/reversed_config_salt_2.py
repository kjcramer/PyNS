...
# =============================================================================
def reversed_config_salt_2(t_h_in,u_h_in,ndt,restart = False):
# -----------------------------------------------------------------------------
    """
    Args:
      t_h_in: ... Hot inlet temperature
      u_h_in: ... Hot channel inlet velocity
      ndt: ...... Total number of time steps
      restart: .. Boolean if the simulation should be restarted from previous saved work space data
    ...
    yn = (nodes(-airgap, 0,12), nodes(-airgap-0.01, -airgap,  48), nodes(0.0, 0.001, 6))
    ...
    x_tot = zeros(vf[H2O].bnd[N].val.shape)
    x_salt = zeros(a[H2O].bnd[N].val.shape)
    ...
    for ts in range(tss,ndt+1):
      ...
      a_2[:,:1,:] = x_salt / (x_tot - m_evap[:,:1,:]*dt) # salt concentration after evaporation
