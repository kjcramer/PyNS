...
yn = (nodes(-0.0005, 0,     2), nodes( -0.0155,    -0.0005,  23), nodes(0, 0.003,  9))
...
dt  =    0.0002  # time step
ndt =   50000       # number of time steps
...
uf[AIR].val[:,:,:] = data['arr_23']
vf[AIR].val[:,:,:] = data['arr_24']
wf[AIR].val[:,:,:] = data['arr_25']
p[AIR].val[:,:,:] = data['arr_26']
...
#  
#  # has nothing to do with the film :) is now between H2O and AIR
...
#  # m_out is positive for condensation and negative for evaporation
...
#  # mem.j is positive for evaporation and negative for condensation  
  ...
  q_t[FIL][:,:1,:]  = -h_d[H2O]*mem.j [:,:1,:] / dv[FIL][:,:1,:]
  q_t[H2O][:,-1:,:] = -h_d[FIL]*m_evap[:,:1,:] / dv[H2O][:,-1:,:]
  ...
  if ts % dt_save == 0:
      ws_name = 'ws_temp_' + id + '.npz'
      np.savez(ws_name, ts, t[AIR].val, t[H2O].val, t[FIL].val,uf[H2O].val,vf[H2O].val,wf[H2O].val,a[H2O].val,a[AIR].val,p[H2O].val,mem.t_int, t_int,m_evap, mem.j, mem.pv,p_v[AIR].val, p_v[AIR].bnd[N].val, p_v[AIR].bnd[S].val, xn, yn[AIR], yn[H2O], yn[FIL], zn, uf[AIR].val,vf[AIR].val,wf[AIR].val,  p[AIR].val)
...
#%%
  if ts % dt_plot == 0:
    ...
    plotname = 'R_plot_' + id + '.png'
    pylab.show()
    pylab.savefig(plotname)
    
    text_id = 'R_Output_' + id + '.txt'
    text_file = open(text_id, "w")
    massflow_for_outfile = np.sum(np.sum(m_evap))/np.sum(np.sum(dx[AIR][:,-1:,:]*dz[AIR][:,-1:,:]))*3600 
    text_file.write("Evaporation mass flux: %3.4e" % massflow_for_outfile)
    text_file.close()
