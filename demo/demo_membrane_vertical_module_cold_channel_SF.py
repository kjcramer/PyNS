...
a_salt = 90.0 # g/l
t_c_in = 20   # C
name = str(t_h_in) + '_SF_2'

# restart options
restart = True
restart_file = 'ws_70_SF_2_temp.npz'
...
for ts in range(tss,ndt+1):
  ...
  t_int, m_evap, t, p_v = calc_interface(t, a, p_v, p_tot, kappa, M, \
                            M_AIR, M_H2O, h_d, (dx,dy,dz), (AIR, FIL), 2.0)  
...
#  mem_diff = mem.j - mem.eps*dx[AIR][:,-1:,:]*dz[AIR][:,-1:,:]/h_d[H2O] \
#      * ( 2.0*kappa[H2O][:,:1,:]/dy[H2O][:,:1,:]*(t[H2O].val[:,:1,:]-mem.t_int) \
#        + 1.0/(dy[AIR][:,-1:,:]/(2.0*kappa[AIR][:,-1:,:])+mem.d/kappa_mem) \
#        * (t[AIR].val[:,-1:,:]-mem.t_int))
#      
#  print("mem_diff = " + "%3.4f" %np.mean(mem_diff))
...
#plt.figure(2);plt.contourf(np.transpose(a[AIR].val[:,:,75]/(M_H2O/M_AIR)*(p_tot[AIR].val[:,:,75]+1e5)/p_v_sat(t[AIR].val[:,:,75])));plt.colorbar()
