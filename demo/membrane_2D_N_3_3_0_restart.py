...
yn = (nodes(-0.0005, 0, 2), nodes( 0,    0.015,  23), nodes(-0.0035, -0.0005,  9))
...
#  mem.j[:,:,:] = C_T[:,:,:] *dx[AIR][:,-1:,:]*dz[AIR][:,-1:,:]*(p_v[AIR].bnd[N].val[:,:,:]-p_v[AIR].val[:,-1:,:])  
  ...
  if ts % dt_save == 0:
      ws_name = 'ws_temp_N_3_3_0' + id + '.npz'
...
#%%
  if ts % dt_plot == 0:
    ...
    plotname = 'plot_' + id + '.png'
    pylab.show()
    pylab.savefig(plotname)
    
    text_id = 'Output_' + id + '.txt'
