...
#                     (rho_max - rho_min) + rho_min
  ...
  if ts % dt_save == 0:
      np.savez('ws_temp_50.npz', ts, t[AIR].val, t[H2O].val, t[FIL].val,uf[H2O].val,vf[H2O].val,wf[H2O].val,a[H2O].val,a[AIR].val,p[H2O].val,mem.t_int, t_int,m_evap, mem.j, mem.pv,p_v[AIR].val, p_v[AIR].bnd[N].val, p_v[AIR].bnd[S].val, uf[AIR].val,vf[AIR].val,wf[AIR].val, xn, yn[AIR], yn[H2O], yn[FIL], zn   )
      if ts % dt_save_ts ==0:
        ws_save_title = 'ws_50_' + str(ts) + 'ts.npz'
